...
Electron 使用 NSIS 完整安装包。Python sidecar 使用 PyInstaller `onedir`，通过 electron-builder `extraResources` 放入安装目录的 `resources/python/`。用户电脑不需要 Node 或 Python。
...
2. PyInstaller sidecar build + JSONL smoke test。
...
Gitee PAT 只能来自本机凭据或 CI Secret，不写入仓库、日志、安装包或命令历史。公开发布前建议配置 Windows 代码签名；未签名构建必须在 Release 中注明 SmartScreen 风险。
