...
from .ready_queue import ReadyQueueEntry, critical_path_chain
...
def build_scheduling_projection(
    *,
    ...
    return {
        ...
        # ★ `readyQueue` 必须是**字符串数组** —— 桌面端的守卫是 fail-closed 的
        #   （isStringArray），给它对象数组会让**整个 scheduling.json 被拒**，
        #   于是 WIP、瓶颈、队列全都显示不出来。
        #   而那个失败在 Python 侧没有任何信号：守卫在 TypeScript 里。
        #   丰富信息另开一个字段 —— 未知键会被忽略，不影响校验。
        "readyQueue": [str(entry.packet_id) for entry in ready_queue],
        "readyQueueDetail": [
            {
                "rank": index + 1,
                "packetId": str(entry.packet_id),
                "criticalPathLength": entry.critical_path,
                "selectedToStart": entry.packet_id in selected_to_start,
            }
            for index, entry in enumerate(ready_queue)
        ],
        # ★ 与 readyQueueDetail 里那个整数不是一回事：这是**链本身**。
        "criticalPath": list(critical_path_chain(packets_by_id)),
        "policy": {
