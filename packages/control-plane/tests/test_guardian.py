...
from codentum_contracts.state import PacketId, RoleSpec, WorkPacket
from codentum_control_plane.guardian import Guardian, Interception


def _pkt(pid="wp-guard01", role="coder", attempts=0):
    return WorkPacket(
...
def _coder_spec():
    return RoleSpec(
        ...
        tools=("t",),
        transitions=({"from":"running","to":"review"},),
        modelPolicy={"defaultModel":"qwen-plus","defaultEffort":"medium"},
    )
...
def _architect_spec():
    return RoleSpec(
        ...
        tools=("t",),
        transitions=({"from":"running","to":"review"},),
        modelPolicy={"defaultModel":"qwen-max","defaultEffort":"high"},
    )
...
class TestI3ContractFreeze:
    ...
    def test_rolespec_write_check(self):
    ...
    def test_rolespec_allows_valid_path(self):
        g = Guardian()
        specs = (_coder_spec(),)
...
class TestInterceptionRecords:
    def test_interceptions_accumulate(self):
        g = Guardian()
    ...
    def test_drain_increments_total(self):
        g = Guardian()
        g.check_path_write(_pkt(role="coder"), "packages/contracts/x.json")
