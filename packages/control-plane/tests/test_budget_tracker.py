...
class TestPersistence:
    def test_to_file(self) -> None:
        bt = BudgetTracker(limit_usd=20.0, degradation_chain=("drop_semantic",))
        bt.spend(5.0, role="coder", model="qwen-plus")
        bf = bt.to_file()
        assert bf.schemaVersion == 1
        assert bf.currency == "USD"
        assert bf.limitUsd == 20.0
        assert bf.spentUsd == 5.0
    ...
    def test_from_real_fixture(self) -> None:
        ...
        bt = BudgetTracker.from_file(bf)
        assert bt.limit_usd == 20.0
        assert bt.spent_usd == pytest.approx(4.82)
